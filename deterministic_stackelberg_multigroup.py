...
# Parametri multigruppo (3 fasce d'eta) per estensione compartimentale minimale
# Nota: la versione base resta valida con i parametri scalari sopra.
N_gruppi_default = np.array([55_000.0, 30_000.0, 15_000.0])
I_t0_gruppi_default = np.array([6.0, 3.0, 1.0])
R_t0_gruppi_default = np.zeros(3)
S_t0_gruppi_default = N_gruppi_default - I_t0_gruppi_default - R_t0_gruppi_default
matrice_contatti_default = np.array([
    [1.00, 0.55, 0.30],
    [0.55, 1.00, 0.45],
    [0.30, 0.45, 1.00],
])
h_gruppi_default = np.array([0.02, 0.05, 0.12])
mu_H_gruppi_default = np.array([0.004, 0.018, 0.075])
...
lambda_reg_controllo_default = 14.0
...
profilo_target_driven = "target-driven"
profilo_esecuzione_default = profilo_prudente
...
kappa_prescrizione_default = 0.05
kappa_prescrizione = kappa_prescrizione_default
...
def _as_1d_float_array(values):
    """Converte input scalare/vettoriale in array 1D float."""
...
def _broadcast_to_groups(values, n_groups, name):
    """Broadcast di un parametro scalare a vettore di lunghezza n_groups."""
    arr = _as_1d_float_array(values)
    if arr.size == 1:
        return np.full(n_groups, float(arr[0]), dtype=float)
    if arr.size != n_groups:
        raise ValueError(f"{name} deve essere scalare oppure lungo {n_groups}.")
    return arr.astype(float)


def _contact_matrix(n_groups, matrice_contatti=None):
    """Ritorna la matrice contatti consistente col numero di gruppi."""
    if matrice_contatti is None:
        if n_groups == 1:
            return np.ones((1, 1), dtype=float)
        if n_groups == len(N_gruppi_default):
            return matrice_contatti_default.astype(float)
        return np.eye(n_groups, dtype=float)

    mat = np.asarray(matrice_contatti, dtype=float)
    if mat.shape != (n_groups, n_groups):
        raise ValueError(
            f"matrice_contatti deve avere shape ({n_groups}, {n_groups})."
        )
    return mat
...
def socialita_prescritta_da_governo(c_s, kappa, tipo="logistica"):
    ...
    """
    c_s_eff = max(0.0, float(c_s))
    kappa_eff = np.asarray(kappa, dtype=float)
    if tipo == "logistica":
        return np.clip(1.0 / (1.0 + kappa_eff * c_s_eff), 0.0, 1.0)
    elif tipo == "esponenziale":
        return np.clip(np.exp(-kappa_eff * c_s_eff), 0.0, 1.0)
    else:
        raise ValueError(f"Tipo prescrizione non riconosciuto: {tipo}")


def rischio_percepito(I_t, N, rho_rischio=rho_rischio_default):
    """
    ...
    """
    I_arr = np.asarray(I_t, dtype=float)
    N_arr = np.asarray(N, dtype=float)
    rho_arr = np.asarray(rho_rischio, dtype=float)
    rischio = rho_arr * (I_arr / N_arr)
    rischio_clipped = np.clip(rischio, 0.0, 1.0)
    if np.ndim(rischio_clipped) == 0:
        return float(rischio_clipped)
    return rischio_clipped
...
def best_response_cittadino_quadratica(
    ...
    """
    if np.all(np.asarray(eta_compliance) > 0):
...
def fattore_contatto_da_socialita(x_t, potenza=potenza_contatto_default):
    """
    ...
    x_t = np.clip(x_t, 0.0, 1.0)
    fattore = x_t ** potenza
    if np.ndim(fattore) == 0:
        return float(fattore)
    return fattore
...
def configura_flag_scansioni(profilo_esecuzione):
    """
    Restituisce la configurazione dei flag di scansione in base al profilo.
    """
    if profilo_esecuzione == profilo_prudente:
        return {
            "esegui_scansione_parametri": False,
            "esegui_scansione_trigger": False,
            "esegui_scansione_comportamento": False,
            "esegui_scansione_target_picco": False,
        }
    if profilo_esecuzione == profilo_target_driven:
        return {
            "esegui_scansione_parametri": False,
            "esegui_scansione_trigger": False,
            "esegui_scansione_comportamento": False,
            "esegui_scansione_target_picco": True,
        }
    raise ValueError(
        f"Profilo non riconosciuto: {profilo_esecuzione}. "
        f"Usare '{profilo_prudente}' o '{profilo_target_driven}'."
    )


flag_scansioni_default = configura_flag_scansioni(profilo_esecuzione_default)
esegui_scansione_parametri_default = flag_scansioni_default["esegui_scansione_parametri"]
esegui_scansione_trigger_default = flag_scansioni_default["esegui_scansione_trigger"]
esegui_scansione_comportamento_default = flag_scansioni_default["esegui_scansione_comportamento"]
esegui_scansione_target_picco_default = flag_scansioni_default["esegui_scansione_target_picco"]
...
def calcola_costo_epidemiologico_istantaneo(
    ...
    # PROMEMORIA TESI: quando verra introdotto il compartimento decessi D,
    # aggiungere qui un termine dedicato (es. costo_decessi_giornaliero * delta_D_t)
    # nella loss istantanea, evitando doppio conteggio sui cumulati.
    _ = S_t
    pressione_ospedaliera = H_t
...
def calcola_costo_epidemiologico_cumulato_con_controllo_variabile(
    ...
    t0,
    giorno_corrente,
    lambda_reg_controllo=lambda_reg_controllo_cumulato_default,
    cap_argomento_exp_saturazione=cap_argomento_exp_saturazione_default,
):
...
def simula_finestra_predizione_stackelberg(
    ...
    a_logaritmica=a_logaritmica_default,
    ...
    """
    N_vec = _as_1d_float_array(N)
    n_groups = len(N_vec)
    S0 = _broadcast_to_groups(S_init, n_groups, "S_init")
    I0 = _broadcast_to_groups(I_init, n_groups, "I_init")
    R0 = _broadcast_to_groups(R_init, n_groups, "R_init")
    H0 = _broadcast_to_groups(H_init, n_groups, "H_init")
    D0 = _broadcast_to_groups(D_init, n_groups, "D_init")
    h_eff = np.clip(_broadcast_to_groups(h, n_groups, "h"), 0.0, 1.0)
    mu_H_eff = np.clip(_broadcast_to_groups(mu_H, n_groups, "mu_H"), 0.0, 1.0)
    rho_eff = _broadcast_to_groups(rho_rischio, n_groups, "rho_rischio")
    eta_eff = _broadcast_to_groups(eta_compliance, n_groups, "eta_compliance")
    kappa_eff = _broadcast_to_groups(kappa_prescrizione, n_groups, "kappa_prescrizione")
    C = _contact_matrix(n_groups, matrice_contatti)

    S_pred = np.zeros((orizzonte + 1, n_groups))
    I_pred = np.zeros((orizzonte + 1, n_groups))
    R_pred = np.zeros((orizzonte + 1, n_groups))
    H_pred = np.zeros((orizzonte + 1, n_groups))
    D_pred = np.zeros((orizzonte + 1, n_groups))
    x_bar_pred = np.zeros((orizzonte + 1, n_groups))
    x_star_pred = np.zeros((orizzonte + 1, n_groups))
    nuovi_infetti_hist = np.zeros((orizzonte, n_groups))
    nuovi_ospedalizzati_hist = np.zeros((orizzonte, n_groups))
    ingressi_in_R_hist = np.zeros((orizzonte, n_groups))

    S_pred[0] = S0
    I_pred[0] = I0
    R_pred[0] = R0
    H_pred[0] = np.maximum(0.0, H0)
    D_pred[0] = np.maximum(0.0, D0)
    ...
    x_bar = socialita_prescritta_da_governo(c_s, kappa_eff, tipo="logistica")
    x_bar_pred[0] = x_bar
    
    for k in range(orizzonte):
        infetti_attivi_k = I_pred[k] + H_pred[k]
        p_k = rischio_percepito(infetti_attivi_k, N_vec, rho_eff)
        ...
        if tipo_best_response == "quadratica":
            ...
            x_star = np.zeros(n_groups)
            for g in range(n_groups):
                x_star[g] = best_response_cittadino_logaritmica(
                    float(x_bar[g]), float(p_k[g]), a_logaritmica, epsilon_logaritmica,
                    lambda_rischio_logaritmica, float(eta_eff[g]), num_grid=num_grid_logaritmica,
                )
        else:
        ...
        prevalenza = np.divide(
            I_pred[k],
            np.maximum(N_vec, 1e-12),
            out=np.zeros_like(I_pred[k]),
            where=np.maximum(N_vec, 1e-12) > 0,
        )
        forza_infezione = beta * fattore * (C @ (fattore * prevalenza))
        new_infections = S_pred[k] * forza_infezione
        ...
        I_pred[k + 1] = np.maximum(0.0, I_pred[k] + new_infections - new_hospitalizations - new_recoveries_non_h)
        H_pred[k + 1] = np.maximum(0.0, H_pred[k] + new_hospitalizations - new_recoveries_h - new_deaths_h)
        R_pred[k + 1] = np.maximum(0.0, R_pred[k] + new_recoveries_non_h + new_recoveries_h - new_waning_immunity)
        D_pred[k + 1] = np.maximum(0.0, D_pred[k] + new_deaths_h)
...
def costo_previsto_su_finestra_stackelberg(
    ...
    if np.ndim(S_pred) == 2:
        S_cost = np.sum(S_pred, axis=1)
        I_cost = np.sum(I_pred, axis=1)
        H_cost = np.sum(H_pred, axis=1)
    else:
        S_cost = S_pred
        I_cost = I_pred
        H_cost = H_pred
...
def simula_sir_stackelberg_con_controllo_periodico(
    ...
    N_vec = _as_1d_float_array(N)
    n_groups = len(N_vec)
    S0 = _broadcast_to_groups(S_t0, n_groups, "S_t0")
    I0 = _broadcast_to_groups(I_t0, n_groups, "I_t0")
    R0 = _broadcast_to_groups(R_t0, n_groups, "R_t0")
    H0 = _broadcast_to_groups(H_init, n_groups, "H_init")
    D0 = _broadcast_to_groups(D_init, n_groups, "D_init")
    h_eff = np.clip(_broadcast_to_groups(h, n_groups, "h"), 0.0, 1.0)
    mu_H_eff = np.clip(_broadcast_to_groups(mu_H, n_groups, "mu_H"), 0.0, 1.0)
    ...
    S = np.zeros((T + 1, n_groups))
    I = np.zeros((T + 1, n_groups))
    R = np.zeros((T + 1, n_groups))
    H = np.zeros((T + 1, n_groups))
    D = np.zeros((T + 1, n_groups))
    nuovi_infetti_hist = np.zeros((T, n_groups))
    nuovi_ospedalizzati_hist = np.zeros((T, n_groups))
    ingressi_in_R_hist = np.zeros((T, n_groups))
    S[0], I[0], R[0] = S0, I0, R0
    H[0] = np.maximum(0.0, H0)
    D[0] = np.maximum(0.0, D0)
    ...
    N_tot = float(np.sum(N_vec))
    soglia_infetti_assoluta = soglia_attivazione_controllo * N_tot  # Soglia in termini di numero assoluto di infetti per attivare il controllo.
    ...
    while giorno < T:
        ...
        infetti_attivi_vett = I[giorno] + H[giorno]
        infetti_attivi = float(np.sum(infetti_attivi_vett))
        ...
        if controllo_attivo and (prossimo_giorno_ottimizzazione is not None) and (giorno >= prossimo_giorno_ottimizzazione): 
            ...
            # !!!Ottimizza c_s sulla finestra di predizione considerando la best response dei cittadini
        ...
        x_bar = socialita_prescritta_da_governo(c_applicato, kappa_eff, tipo="logistica")
        p_t = rischio_percepito(infetti_attivi_vett, N_vec, rho_eff)
        ...
        if tipo_best_response == "quadratica":
            ...
            for g in range(n_groups):
                x_star[g] = best_response_cittadino_logaritmica(
                    float(x_bar[g]), float(p_t[g]), a_logaritmica, epsilon_logaritmica,
                    lambda_rischio_logaritmica, float(eta_eff[g]), num_grid=num_grid_logaritmica,
                )
        ...
        prevalenza = np.divide(
            I[giorno],
            np.maximum(N_vec, 1e-12),
            out=np.zeros_like(I[giorno]),
            where=np.maximum(N_vec, 1e-12) > 0,
        )
        forza_infezione = beta * fattore * (C @ (fattore * prevalenza))
        new_infections = S[giorno] * forza_infezione
        ...
        I[giorno + 1] = np.maximum(0.0, I[giorno] + new_infections - new_hospitalizations - new_recoveries_non_h)
        H[giorno + 1] = np.maximum(0.0, H[giorno] + new_hospitalizations - new_recoveries_h - new_deaths_h)
        R[giorno + 1] = np.maximum(0.0, R[giorno] + new_recoveries_non_h + new_recoveries_h - new_waning_immunity)
        D[giorno + 1] = np.maximum(0.0, D[giorno] + new_deaths_h)
        
        c_schedule[giorno] = c_applicato
        x_bar_schedule[giorno] = float(np.sum(x_bar * N_vec) / max(N_tot, 1e-12))
        x_star_schedule[giorno] = float(np.sum(x_star * N_vec) / max(N_tot, 1e-12))
        p_rischio_schedule[giorno] = float(np.sum(p_t * N_vec) / max(N_tot, 1e-12))
    ...
    if n_groups == 1:
        return S[:, 0], I[:, 0], R[:, 0], H[:, 0], D[:, 0], c_schedule, x_bar_schedule, x_star_schedule, p_rischio_schedule, log_ottimizzazione

    S_agg = np.sum(S, axis=1)
    I_agg = np.sum(I, axis=1)
    R_agg = np.sum(R, axis=1)
    H_agg = np.sum(H, axis=1)
    D_agg = np.sum(D, axis=1)

    if ritorna_traiettorie_per_gruppo:
        info_gruppi = {
            "S_gruppi": S,
            "I_gruppi": I,
            "R_gruppi": R,
        ...
        return (
            S_agg, I_agg, R_agg, H_agg, D_agg,
            c_schedule, x_bar_schedule, x_star_schedule, p_rischio_schedule,
            log_ottimizzazione, info_gruppi,
        )

    return S_agg, I_agg, R_agg, H_agg, D_agg, c_schedule, x_bar_schedule, x_star_schedule, p_rischio_schedule, log_ottimizzazione
...
def esegui_scansione_target_picco(
    ...
    target_picco_percent=target_picco_percent_default,
    tolleranza_percent=tolleranza_target_percent_default,
):
    """
...
def esegui_confronto_quadratica_vs_logaritmica():
    """
    Esegue le due simulazioni, calcola metriche comparative e stampa il report.
    """
    ...
    (
        ...
        x_star_quadratica,
        p_quadratica,
        log_ottimizzazioni_quadratica,
    ) = simula_sir_stackelberg_con_controllo_periodico(
        S_t0, I_t0, R_t0, N, beta, gamma, T,
        costo_infetto_giornaliero, k_saturazione_ospedali, alpha_saturazione_ospedali,
    ...
    (
        ...
        x_star_logaritmica,
        p_logaritmica,
        log_ottimizzazioni_logaritmica,
    ) = simula_sir_stackelberg_con_controllo_periodico(
        S_t0, I_t0, R_t0, N, beta, gamma, T,
        costo_infetto_giornaliero, k_saturazione_ospedali, alpha_saturazione_ospedali,
        c_iniziale=c_iniziale_default,
    ...
    giorno_picco_quadratica = int(I_quadratica.argmax())
    giorno_picco_logaritmica = int(I_logaritmica.argmax())
    picco_infetti_quadratica = float(I_quadratica.max())
    picco_infetti_logaritmica = float(I_logaritmica.max())

    costo_totale_quadratica = calcola_costo_epidemiologico_cumulato_con_controllo_variabile(
        S_quadratica, I_quadratica, H_quadratica, costo_infetto_giornaliero, c_quadratica, k_saturazione_ospedali,
        alpha_saturazione_ospedali, 0, T, lambda_reg_controllo,
    )
    costo_totale_logaritmica = calcola_costo_epidemiologico_cumulato_con_controllo_variabile(
        S_logaritmica, I_logaritmica, H_logaritmica, costo_infetto_giornaliero, c_logaritmica, k_saturazione_ospedali,
        alpha_saturazione_ospedali, 0, T, lambda_reg_controllo,
    )

    compliance_media_quadratica = np.mean([
        x_star_quadratica[i] / max(x_bar_quadratica[i], 0.01)
        for i in range(len(x_star_quadratica))
    ]) if T > 0 else np.nan
    compliance_media_logaritmica = np.mean([
        x_star_logaritmica[i] / max(x_bar_logaritmica[i], 0.01)
        for i in range(len(x_star_logaritmica))
    ]) if T > 0 else np.nan
    gap_medio_quadratica = float(np.mean(x_star_quadratica - x_bar_quadratica))
    gap_medio_logaritmica = float(np.mean(x_star_logaritmica - x_bar_logaritmica))
    giorno_picco_ospedalizzati_quadratica = int(np.argmax(H_quadratica))
    ...
    if verbose_progress_default:
        print(f"\nNumero ottimizzazioni MPC quadratica: {len(log_ottimizzazioni_quadratica)}")
        print(f"Numero ottimizzazioni MPC logaritmica: {len(log_ottimizzazioni_logaritmica)}")
    ...
    return {
        "quadratica": {
            ...
            "p": p_quadratica,
            "giorno_picco": giorno_picco_quadratica,
        },
        "logaritmica": {
            "S": S_logaritmica,
            "I": I_logaritmica,
            ...
            "p": p_logaritmica,
            "giorno_picco": giorno_picco_logaritmica,
        },
    }


def genera_grafici_confronto(risultati_confronto):
    """
    Genera i grafici usando i risultati gia' calcolati nel confronto.
    """
    quadratica = risultati_confronto["quadratica"]
    logaritmica = risultati_confronto["logaritmica"]
    ...
    plot_controllo_e_comportamento_stackelberg(
        t, quadratica["S"], quadratica["I"], quadratica["c"], quadratica["x_bar"], quadratica["x_star"], quadratica["p"],
        H=quadratica["H"],
        simulation_label="quadratica",
    ...
    plot_controllo_e_comportamento_stackelberg(
        t, logaritmica["S"], logaritmica["I"], logaritmica["c"], logaritmica["x_bar"], logaritmica["x_star"], logaritmica["p"],
        H=logaritmica["H"],
        simulation_label="logaritmica",
